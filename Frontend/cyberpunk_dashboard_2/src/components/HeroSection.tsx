...
export const HeroSection: FC<HeroSectionProps> = ({ onStart }) => {
  const [showMap, setShowMap] = useState(false);
  const particles = useMemo(
    () =>
      Array.from({ length: 40 }, (_, index) => {
        const size = 2 + getParticleValue(index, 1) * 2;
        const style: ParticleStyle = {
          left: `${getParticleValue(index, 2) * 100}%`,
          top: `${getParticleValue(index, 3) * 100}%`,
          width: `${size}px`,
          height: `${size}px`,
          opacity: 0.18 + getParticleValue(index, 4) * 0.32,
          animationDuration: `${12 + getParticleValue(index, 5) * 10}s`,
          animationDelay: `${getParticleValue(index, 6) * 8}s`,
          '--particle-drift': `${(getParticleValue(index, 7) - 0.5) * 40}px`,
          '--particle-opacity': `${0.25 + getParticleValue(index, 8) * 0.35}`,
        };
  ...
  return (
    <section className="relative w-full h-screen flex items-center justify-center overflow-hidden bg-bg-primary">
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-neon-primary/5 blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-accent-blue/5 blur-[120px] pointer-events-none" />
      ...
      <motion.div
        ...
        className="absolute top-6 left-8 z-20 text-neon-primary text-xl font-bold uppercase tracking-widest drop-shadow-[0_0_8px_rgba(198,255,0,0.5)]"
      >
        TRAXIS
      </motion.div>

      <div className="absolute top-6 right-8 z-20 flex items-center gap-4 text-sm font-mono tracking-[0.18em] uppercase text-white/60 pointer-events-none">
        <div className="flex items-center gap-2">
          <motion.span
            animate={{ scale: [1, 1.2, 1], opacity: [0.8, 1, 0.8] }}
            transition={{ duration: 2.2, repeat: Number.POSITIVE_INFINITY, ease: 'easeInOut' }}
            className="h-2 w-2 rounded-full bg-neon-primary shadow-[0_0_10px_rgba(198,255,0,0.8)]"
          />
          <span>SYSTEM ONLINE</span>
        </div>
        <span className="text-white/25">|</span>
        <div className="flex items-center gap-2">
          <MapPin size={14} className="text-neon-primary drop-shadow-[0_0_6px_rgba(198,255,0,0.8)]" strokeWidth={1.5} />
          <span>DELHI NODE</span>
        </div>
        <span className="text-white/25">|</span>
        <div className="flex items-center gap-2">
          <User size={14} className="text-neon-primary drop-shadow-[0_0_6px_rgba(198,255,0,0.8)]" strokeWidth={1.5} />
          <span>USER</span>
        </div>
      </div>

      <div className="max-w-7xl w-full px-6 flex flex-col lg:flex-row items-center justify-between gap-12 z-10">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, ease: "easeOut" }}
          className="flex-1 lg:max-w-[45%] flex flex-col items-start text-left"
        >
          <div className="flex flex-col w-full space-y-4">
            <motion.h1 
              ...
              className="text-2xl md:text-3xl lg:text-4xl font-bold tracking-[0.24em] uppercase text-white/90 leading-tight whitespace-nowrap"
            >
              SMART TRAVEL
            </motion.h1>
            ...
            <motion.h2 
              ...
              className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter uppercase neon-text text-neon-primary leading-[0.9] whitespace-nowrap"
            >
              INTELLIGENCE
            </motion.h2>
          </div>
          ...
          <motion.p 
            ...
            className="mt-6 text-text-secondary text-lg max-w-md font-medium"
          >
            Navigate cities with AI-powered real-time decision intelligence.
          </motion.p>
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.05, boxShadow: "0 0 20px var(--color-neon-primary)" }}
            whileTap={{ scale: 0.95 }}
            onClick={onStart}
            className="mt-10 px-8 py-4 bg-neon-primary text-black font-black tracking-widest uppercase rounded-sm transition-all duration-300 hover:bg-neon-secondary"
          >
            START ANALYSIS
          </motion.button>
        </motion.div>
        ...
        {showMap && (
          <motion.div
            ...
            className="flex-1 relative aspect-square lg:aspect-video w-full"
          >
            <div className="w-full h-full bg-black/20 border border-neon-primary/20 rounded-xl relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-br from-neon-primary/5 via-transparent to-accent-blue/10 opacity-70" />
              <div className="absolute top-0 left-0 w-4 h-4 border-t-2 border-l-2 border-neon-primary/40" />
              <div className="absolute top-0 right-0 w-4 h-4 border-t-2 border-r-2 border-neon-primary/40" />
              <div className="absolute bottom-0 left-0 w-4 h-4 border-b-2 border-l-2 border-neon-primary/40" />
              <div className="absolute bottom-0 right-0 w-4 h-4 border-b-2 border-r-2 border-neon-primary/40" />

              <div className="absolute top-5 left-5 z-20 rounded-full border border-neon-primary/20 bg-black/40 px-3 py-1 text-[10px] font-mono tracking-[0.28em] uppercase text-neon-primary/70">
                India Nexus
              </div>
              ...
              <div className="relative w-full h-full">
                <motion.div
                  ...
                  animate={{ opacity: 0.22, scale: 2.5, x: '10%', y: '10%' }}
                  transition={{ delay: 0.9, duration: 0.65, ease: 'easeInOut' }}
                  className="absolute inset-0 flex items-center justify-center"
                  style={{ transformOrigin: targetPoint.origin }}
                >
                  <div className="w-[74%] max-w-[26rem]">
                    <svg
                      viewBox="0 0 666.66669 777.33331"
                      preserveAspectRatio="xMidYMid meet"
                      className="w-full h-auto"
                      style={{ filter: 'drop-shadow(0 0 14px rgba(198, 255, 0, 0.24))' }}
                    >
                    <motion.path
                      d={indiaOutlinePath}
                      fill="none"
                    ...
                    <motion.circle
                      ...
                      cy={targetPoint.y}
                      r="4.8"
                      fill="rgba(198, 255, 0, 0.22)"
                      initial={{ opacity: 0, scale: 0.85 }}
                      animate={{ opacity: 0.65, scale: 1 }}
                      transition={{ delay: 0.25, duration: 0.22, ease: 'easeInOut' }}
                      style={{ transformBox: 'fill-box', transformOrigin: 'center', filter: 'blur(3px)' }}
                    />
                    <motion.circle
                      cx={targetPoint.x}
                      cy={targetPoint.y}
                      ...
                      animate={{ opacity: 1, scale: 1 }}
                      transition={{ delay: 0.25, duration: 0.22, ease: 'easeInOut' }}
                      style={{ transformBox: 'fill-box', transformOrigin: 'center', filter: 'drop-shadow(0 0 8px rgba(198, 255, 0, 0.95))' }}
                    />
                    <motion.circle
                      cx={targetPoint.x}
                      cy={targetPoint.y}
                      r="3.8"
                      fill="none"
                      stroke="rgba(198, 255, 0, 0.72)"
                      strokeWidth="0.95"
                      initial={{ opacity: 0, scale: 0.7 }}
                      animate={{ opacity: [0, 0.65, 0], scale: [0.7, 2.05, 3.05] }}
                      transition={{ delay: 0.25, duration: 1.8, ease: 'easeOut', repeat: Number.POSITIVE_INFINITY, repeatDelay: 0.2 }}
                    ...
                    <motion.circle
                      ...
                      animate={{ opacity: [0, 0.34, 0], scale: [0.8, 1.8, 2.45] }}
                      transition={{ delay: 0.37, duration: 2.1, ease: 'easeOut', repeat: Number.POSITIVE_INFINITY, repeatDelay: 0.25 }}
                      style={{ transformBox: 'fill-box', transformOrigin: 'center' }}
                    />
                    </svg>
                  </div>
                ...
                >
                  <div className="glass w-full rounded-2xl border border-white/10 bg-black/35 p-5 backdrop-blur-xl">
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        <div className="text-[10px] font-mono tracking-[0.24em] uppercase text-accent-blue">Target Lock</div>
                        <div className="mt-2 text-2xl font-black uppercase tracking-[0.16em] text-neon-primary neon-text">Delhi</div>
                      </div>
                      <div className="rounded-full border border-neon-primary/20 bg-neon-primary/10 px-3 py-1 text-[10px] font-mono tracking-[0.22em] uppercase text-neon-primary">
                        Live
                      </div>
                    </div>
                    <div className="mt-4 grid grid-cols-3 gap-3 text-left">
                      <div className="rounded-xl border border-white/8 bg-white/4 p-3">
                        <div className="text-[9px] font-mono uppercase tracking-[0.22em] text-white/45">Node</div>
                        <div className="mt-2 text-sm font-semibold text-white">NCR-01</div>
                      </div>
                      <div className="rounded-xl border border-white/8 bg-white/4 p-3">
                        <div className="text-[9px] font-mono uppercase tracking-[0.22em] text-white/45">Feed</div>
                        <div className="mt-2 text-sm font-semibold text-white">Transit Grid</div>
                      </div>
                      <div className="rounded-xl border border-white/8 bg-white/4 p-3">
                        <div className="text-[9px] font-mono uppercase tracking-[0.22em] text-white/45">Sync</div>
                        <div className="mt-2 text-sm font-semibold text-white">Realtime</div>
                      </div>
                    </div>
                  </div>
                </motion.div>
      ...
      >
        ...
        >
          <ArrowDown size={16} className="text-neon-primary/60" />
        </motion.div>
      </motion.div>
    </section>
  );
